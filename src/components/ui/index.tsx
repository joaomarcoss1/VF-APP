...
export function Button({ children, onClick, type = 'button', variant = 'primary', size = 'md', disabled, loading, className = '', fullWidth }: { children: ReactNode; onClick?: () => void; type?: 'button'|'submit'|'reset'; variant?: BtnVariant; size?: BtnSize; disabled?: boolean; loading?: boolean; className?: string; fullWidth?: boolean }) {
  ...
  const variants = {
    ...
    danger: 'bg-[color-mix(in_srgb,var(--vf-error)_10%,transparent)] hover:bg-[color-mix(in_srgb,var(--vf-error)_16%,transparent)] border border-[color-mix(in_srgb,var(--vf-error)_28%,transparent)] text-[var(--vf-error)]',
  }
...
export function Modal({ open, onClose, title, children, size = 'md' }: { open: boolean; onClose: () => void; title: string; children: ReactNode; size?: 'sm'|'md'|'lg'|'xl' }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4"><div className="absolute inset-0 bg-[var(--vf-overlay)] backdrop-blur-sm" onClick={onClose} /><div className={`relative w-full ${widths[size]} bg-[var(--vf-surface)] border border-[var(--vf-border)] rounded-t-3xl md:rounded-2xl shadow-2xl vf-fadein max-h-[92vh] flex flex-col`}><div className="flex items-center justify-between px-5 py-4 border-b border-[var(--vf-border)]"><h2 className="text-[15px] font-semibold text-[var(--vf-text)]">{title}</h2><button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-[var(--vf-surface2)] text-[var(--vf-text3)] hover:text-[var(--vf-text)] transition-colors text-lg">✕</button></div><div className="overflow-y-auto flex-1 p-5">{children}</div></div></div>
}
...
export function Badge({ children, color = 'gold' }: { children: ReactNode; color?: BadgeColor }) {
  const colors = { gold: 'bg-[color-mix(in_srgb,var(--vf-secondary)_18%,transparent)] text-[var(--vf-secondary)] border-[color-mix(in_srgb,var(--vf-secondary)_32%,transparent)]', green: 'bg-[color-mix(in_srgb,var(--vf-success)_12%,transparent)] text-[var(--vf-success)] border-[color-mix(in_srgb,var(--vf-success)_25%,transparent)]', red: 'bg-[color-mix(in_srgb,var(--vf-error)_10%,transparent)] text-[var(--vf-error)] border-[color-mix(in_srgb,var(--vf-error)_25%,transparent)]', blue: 'bg-[color-mix(in_srgb,var(--vf-primary)_14%,transparent)] text-[var(--vf-primary)] border-[color-mix(in_srgb,var(--vf-primary)_28%,transparent)]', amber: 'bg-[color-mix(in_srgb,var(--vf-warning)_12%,transparent)] text-[var(--vf-warning)] border-[color-mix(in_srgb,var(--vf-warning)_26%,transparent)]', gray: 'bg-[color-mix(in_srgb,var(--vf-text3)_12%,transparent)] text-[var(--vf-text3)] border-[color-mix(in_srgb,var(--vf-text3)_18%,transparent)]' }
...
export function KpiCard({ label, value, delta, deltaUp, prefix = '', suffix = '', color = 'gold' }: { label: string; value: string | number; delta?: string; deltaUp?: boolean; prefix?: string; suffix?: string; color?: 'gold'|'green'|'red'|'blue' }) {
  const colors = { gold: 'var(--vf-secondary)', green: 'var(--vf-success)', red: 'var(--vf-error)', blue: 'var(--vf-primary)' }
  return <div className="vf-card p-4 relative overflow-hidden"><div className="absolute top-0 left-0 right-0 h-[3px]" style={{ background: colors[color], opacity: .7 }} /><div className="text-[10px] text-[var(--vf-text3)] uppercase tracking-wider mb-1.5">{label}</div><div className="text-2xl font-semibold mb-1" style={{ color: colors[color] }}>{prefix}{typeof value === 'number' ? value.toLocaleString('pt-BR') : value}{suffix}</div>{delta && <div className={`text-[11px] flex items-center gap-1 ${deltaUp ? 'text-[var(--vf-success)]' : 'text-[var(--vf-error)]'}`}><span>{deltaUp ? '↑' : '↓'}</span><span>{delta}</span></div>}</div>
}
...
export function Alert({ type, children }: { type: 'info'|'warn'|'error'|'success'; children: ReactNode }) { const styles = { info: { bg: 'color-mix(in srgb, var(--vf-info) 9%, transparent)', border: 'color-mix(in srgb, var(--vf-info) 26%, transparent)', icon: 'ℹ️' }, warn: { bg: 'color-mix(in srgb, var(--vf-warning) 11%, transparent)', border: 'color-mix(in srgb, var(--vf-warning) 30%, transparent)', icon: '⚠️' }, error: { bg: 'color-mix(in srgb, var(--vf-error) 9%, transparent)', border: 'color-mix(in srgb, var(--vf-error) 26%, transparent)', icon: '✕' }, success: { bg: 'color-mix(in srgb, var(--vf-success) 11%, transparent)', border: 'color-mix(in srgb, var(--vf-success) 28%, transparent)', icon: '✓' } }; const s = styles[type]; return <div className="flex items-start gap-2.5 p-3 rounded-2xl text-[13px] text-[var(--vf-text)]" style={{ background: s.bg, border: `1px solid ${s.border}` }}><span className="flex-shrink-0">{s.icon}</span><div>{children}</div></div> }
