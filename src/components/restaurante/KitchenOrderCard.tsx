  ...
  const style = order.status === 'novo'
    ...
    : order.status === 'em_preparo'
      ...
      : order.status === 'pronto'
        ...
        : 'border-[var(--vf-border)] bg-[var(--vf-card)] shadow-slate-100'
  return (
    <article className={`rounded-[26px] border p-5 shadow-xl transition-all duration-300 ${style}`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <span className="text-xs font-black uppercase tracking-[.18em] text-[var(--vf-text3)]">Mesa</span>
          <h3 className="text-3xl font-black text-[var(--vf-text)]">{order.mesa_numero ?? 'Balcão'}</h3>
          <p className="text-xs font-bold text-[var(--vf-text3)]">Comanda #{order.codigo_comanda ?? order.comanda_id.slice(0, 6)}</p>
        </div>
        <StatusBadge status={order.status} />
      </div>
      <div className="my-4 space-y-2 rounded-2xl bg-[var(--vf-card)]/76 p-3 ring-1 ring-black/5">
        {(order.itens ?? []).map((item) => (
          <div key={item.id} className="flex justify-between gap-3 text-sm">
            <span className="font-extrabold text-[var(--vf-text)]">{item.quantidade}x {item.nome_produto}</span>
            {item.observacao && <span className="max-w-[42%] text-right text-xs font-semibold text-[var(--vf-text3)]">{item.observacao}</span>}
          </div>
        ))}
      </div>
      ...
      {order.status === 'retirado' && <div className="rounded-2xl bg-[var(--vf-card)]/70 p-3 text-center text-sm font-black text-[var(--vf-text3)]">Finalizado</div>}
    </article>
  )
}
