    ...
    <section className="grid gap-4 lg:grid-cols-[220px,1fr]">
      <div className="space-y-2">
        {paymentOptions.map((option) => {
          ...
          return <button key={option.id} onClick={() => setForma(option.id)} className={`flex w-full items-center gap-3 rounded-2xl border px-4 py-3 text-left text-sm font-black transition ${active ? 'border-emerald-300 bg-emerald-50 text-emerald-700 shadow-sm' : 'border-[var(--vf-border)] bg-[var(--vf-card)] text-[var(--vf-text2)] hover:border-blue-200 hover:bg-blue-50'}`}><Icon size={17} /> {option.label}</button>
        })}
      </div>
      <div className="rounded-[26px] border border-[var(--vf-border)] bg-[var(--vf-card)] p-5 shadow-sm">
        <div className="grid gap-3 md:grid-cols-3">
          <div className="rounded-2xl bg-[var(--vf-surface2)] p-4">
            <span className="text-xs font-black uppercase tracking-wide text-[var(--vf-text3)]">Total</span>
            <strong className="mt-1 block text-xl font-black text-[var(--vf-text)]">{formatCurrency(total)}</strong>
          </div>
          <label className="rounded-2xl border border-[var(--vf-border)] p-4">
            <span className="text-xs font-black uppercase tracking-wide text-[var(--vf-text3)]">Valor</span>
          ...
          <label className="rounded-2xl border border-[var(--vf-border)] p-4">
            <span className="text-xs font-black uppercase tracking-wide text-[var(--vf-text3)]">Recebido</span>
            <input value={recebido} onChange={(e) => setRecebido(Number(e.target.value))} type="number" step="0.01" disabled={forma !== 'dinheiro'} className="mt-2 w-full border-0 bg-transparent text-xl font-black outline-none disabled:text-[var(--vf-text3)]" />
          </label>
        </div>
        ...
        <div className="mt-5 rounded-2xl border border-[var(--vf-border)] bg-[var(--vf-surface2)] p-4">
          <div className="mb-3 flex items-center justify-between"><strong>Pagamentos</strong><span className="text-sm font-black text-[var(--vf-text3)]">Restante: {formatCurrency(summary.restante)}</span></div>
          {payments.length === 0 ? <p className="text-sm font-semibold text-[var(--vf-text3)]">Nenhum pagamento lançado.</p> : payments.map((p, index) => <div key={index} className="flex items-center justify-between border-t border-[var(--vf-border)] py-2 text-sm"><span className="font-bold capitalize">{p.forma_pagamento}</span><strong>{formatCurrency(p.valor)}</strong></div>)}
        </div>
