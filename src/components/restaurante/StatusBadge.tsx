...
const config: Record<string, { label: string; className: string; icon: any }> = {
  ...
  bloqueada: { label: 'Bloqueada', className: 'bg-slate-100 text-slate-600 border-slate-200', icon: Lock },
  ...
  retirado: { label: 'Retirado', className: 'bg-slate-50 text-slate-600 border-slate-200', icon: CheckCircle2 },
}

export function StatusBadge({ status, className = '' }: { status: string; className?: string }) {
  const item = config[status] ?? { label: status, className: 'bg-slate-50 text-slate-700 border-slate-200', icon: Clock3 }
