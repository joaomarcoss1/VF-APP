...
export const VarejoService = {
  async listarVariacoes(produtoId?: string): Promise<ProdutoVariacao[]> {
    ...
    let q = db().from('produto_variacoes').select('*').eq('empresa_id', empresaId).eq('ativo', true).order('created_at', { ascending: false })
  ...
  async salvarVariacao(form: Partial<ProdutoVariacao> & { produto_id: string }): Promise<ProdutoVariacao> {
    ...
    const request = form.id
      ? db().from('produto_variacoes').update(payload).eq('id', form.id).eq('empresa_id', payload.empresa_id).select().single()
  ...
  async desativarVariacao(id: string): Promise<void> {
    ...
    const empresaId = await getEmpresaId()
    const { error } = await db().from('produto_variacoes').update({ ativo: false, updated_at: new Date().toISOString() }).eq('id', id).eq('empresa_id', empresaId)
  ...
  async estoqueVariacoes(produtoId?: string): Promise<ProdutoVariacaoEstoque[]> {
    const empresaId = await getEmpresaId()
    let q = db().from('produto_variacao_estoque').select('*, variacao:produto_variacoes(*)').eq('empresa_id', empresaId).order('updated_at', { ascending: false })
