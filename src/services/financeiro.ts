...
import { db, hojeISO, normalizeEmptyValues, normalizeError, withEmpresa, assertPermission, type AnyRecord } from './_base'
...
export const FinanceiroService = {
  async listar(inicio?: string, fim?: string): Promise<LancamentoFinanceiro[]> {
    ...
    let q = db().from('lancamentos_financeiros').select('*').eq('empresa_id', empresaId).order('data_vencimento', { ascending: false })
  ...
  async atualizar(id: string, form: Partial<LancamentoFinanceiroForm>): Promise<LancamentoFinanceiro> {
    ...
    const empresaId = await getEmpresaIdObrigatoria()
    const { data, error } = await db().from('lancamentos_financeiros').update(payload).eq('id', id).eq('empresa_id', empresaId).select().single()
  ...
  async excluir(id: string): Promise<void> {
    ...
    const empresaId = await getEmpresaIdObrigatoria()
    const { error } = await db().from('lancamentos_financeiros').update({ status: 'cancelado', updated_at: new Date().toISOString() }).eq('id', id).eq('empresa_id', empresaId)
  ...
  async marcarPago(id: string): Promise<void> {
    await assertPermission('financeiro', 'editar')
    const empresaId = await getEmpresaIdObrigatoria()
    const { error } = await db().from('lancamentos_financeiros').update({ status: 'pago', data_pagamento: hojeISO(), updated_at: new Date().toISOString() }).eq('id', id).eq('empresa_id', empresaId)
...
export const ContasPagarService = {
  async listar(inicio?: string, fim?: string): Promise<ContaPagar[]> {
    const empresaId = await getEmpresaIdObrigatoria()
    let q = db().from('contas_pagar').select('*').eq('empresa_id', empresaId).order('data_vencimento', { ascending: true })
    if (inicio) q = q.gte('data_vencimento', inicio)
    if (fim) q = q.lte('data_vencimento', fim)
    const { data, error } = await q
  ...
  async marcarPago(id: string): Promise<void> {
    await assertPermission('financeiro', 'editar')
    const empresaId = await getEmpresaIdObrigatoria()
    const { error } = await db().from('contas_pagar').update({ status: 'pago', data_pagamento: hojeISO(), updated_at: new Date().toISOString() }).eq('id', id).eq('empresa_id', empresaId)
...
export const ContasReceberService = {
  async listar(inicio?: string, fim?: string): Promise<ContaReceber[]> {
    const empresaId = await getEmpresaIdObrigatoria()
    let q = db().from('contas_receber').select('*').eq('empresa_id', empresaId).order('data_vencimento', { ascending: true })
    if (inicio) q = q.gte('data_vencimento', inicio)
    if (fim) q = q.lte('data_vencimento', fim)
    const { data, error } = await q
  ...
  async marcarRecebido(id: string): Promise<void> {
    await assertPermission('financeiro', 'editar')
    const empresaId = await getEmpresaIdObrigatoria()
    const { error } = await db().from('contas_receber').update({ status: 'recebido', data_recebimento: hojeISO(), updated_at: new Date().toISOString() }).eq('id', id).eq('empresa_id', empresaId)
