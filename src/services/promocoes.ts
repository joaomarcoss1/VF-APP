...
export const PromocoesService = {
  ...
  async listar(): Promise<Promocao[]> {
    ...
    const { data, error } = await db().from('promocoes').select('*, produto:produtos(*)').eq('empresa_id', empresaId).order('created_at', { ascending: false })
  ...
  async atualizar(id: string, form: Partial<PromocaoForm>): Promise<Promocao> {
    ...
    const { data, error } = await db().from('promocoes').update(payload).eq('empresa_id', await getEmpresaIdObrigatoria()).eq('id', id).select('*, produto:produtos(*)').single()
  ...
  async excluir(id: string): Promise<void> {
    ...
    const { error } = await db().from('promocoes').delete().eq('empresa_id', await getEmpresaIdObrigatoria()).eq('id', id)
