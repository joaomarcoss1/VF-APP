...
export const DespesasService = {
  async listar(): Promise<Despesa[]> {
    ...
    const { data, error } = await db().from('despesas').select('*').eq('empresa_id', empresaId).order('created_at', { ascending: false })
  ...
  async atualizar(id: string, form: Partial<DespesaForm>): Promise<Despesa> {
    ...
    const { data, error } = await db().from('despesas').update(payload).eq('empresa_id', await getEmpresaIdObrigatoria()).eq('id', id).select().single()
  ...
  async excluir(id: string): Promise<void> {
    ...
    const { error } = await db().from('despesas').delete().eq('empresa_id', await getEmpresaIdObrigatoria()).eq('id', id)
