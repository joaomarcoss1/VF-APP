...
async function assertEventoEmpresa(eventoId: string, empresaId: string): Promise<void> {
  const { data, error } = await db()
    ...
    .maybeSingle()
  if (error) throw normalizeError(error, 'Erro ao validar evento da empresa.')
  if (!data) throw new Error('Evento não encontrado nesta empresa ou sem permissão de acesso.')
}
...
export const EventosService = {
  ...
  async buscar(id: string): Promise<Evento | null> {
    const empresaId = await getEmpresaIdObrigatoria()
    const { data, error } = await db()
      .from('eventos')
      .select('*, itens:evento_itens(*)')
      ...
      .eq('empresa_id', empresaId)
      .maybeSingle()
    if (error) throw normalizeError(error, 'Erro ao buscar evento.')
  ...
  async criar(form: EventoForm): Promise<Evento> {
    ...
    const empresaId = await getEmpresaIdObrigatoria()
    const calc = calcularPrecificacaoEvento({
      pessoas: form.pessoas,
      margem_lucro: form.margem_lucro,
      taxa_operacional_percentual: form.taxa_operacional_percentual,
      custo_operacional_extra: form.custo_operacional_extra,
      desconto: form.desconto,
      itens: form.itens as EventoItemForm[],
    })
    const payload = normalizeEmptyValues({
      ...
      pessoas: calc.pessoas,
      margem_lucro: calc.margem_lucro,
      taxa_operacional_percentual: calc.taxa_operacional_percentual,
      custo_operacional_extra: calc.custo_operacional_extra,
      desconto: calc.desconto,
      custo_produtos: calc.custo_produtos,
      custo_total: calc.custo_total,
      preco_sugerido: calc.preco_sugerido,
      preco_por_pessoa: calc.preco_por_pessoa,
      lucro_estimado: calc.lucro_estimado,
      cmv_percentual: calc.cmv_percentual,
  ...
  async atualizar(id: string, form: EventoForm): Promise<Evento> {
    ...
    const empresaId = await getEmpresaIdObrigatoria()
    await assertEventoEmpresa(id, empresaId)
    const calc = calcularPrecificacaoEvento({
      pessoas: form.pessoas,
      margem_lucro: form.margem_lucro,
      taxa_operacional_percentual: form.taxa_operacional_percentual,
    ...
    await db().from('evento_itens').delete().eq('empresa_id', empresaId).eq('evento_id', id)
    if (calc.itens.length) {
  ...
  async excluir(id: string): Promise<void> {
    ...
    await assertEventoEmpresa(id, empresaId)
    await db().from('evento_itens').delete().eq('empresa_id', empresaId).eq('evento_id', id)
    const { error } = await db().from('eventos').delete().eq('id', id).eq('empresa_id', empresaId)
