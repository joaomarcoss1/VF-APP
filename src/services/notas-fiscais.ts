...
export const NotasFiscaisService = {
  async listar(limit = 50): Promise<NotaFiscalBase[]> {
    ...
    const { data, error } = await db().from('notas_fiscais').select('*, itens:nota_fiscal_itens(*)').eq('empresa_id', empresaId).order('created_at', { ascending: false }).limit(limit)
  ...
  async criar(form: NotaFiscalForm): Promise<NotaFiscalBase> {
    ...
    const rpcPayload = { nota: notaPayload, itens }
    const { data: rpcNota, error: rpcError } = await db().rpc('vf_registrar_nota_abastecimento_v12', { p_payload: rpcPayload })
    const rpcMissing = Boolean(rpcError && ((rpcError as any).code === 'PGRST202' || String((rpcError as any).message || '').toLowerCase().includes('function')))
    if (rpcError && !rpcMissing) throw normalizeError(rpcError, 'Erro ao salvar nota fiscal de forma transacional.')
    if (!rpcError && rpcNota) {
      await AuditoriaService.registrar('notas_fiscais.criar.transacional', 'notas_fiscais', (rpcNota as any).id, { numero: (rpcNota as any).numero, valor_total: (rpcNota as any).valor_total }).catch(() => null)
      return rpcNota as NotaFiscalBase
    }
  ...
  async exportarModeloCSV(): Promise<void> {
    downloadWorkbookXlsx({
      'Modelo de importação': [{ numero: '0001', fornecedor_nome: 'Fornecedor exemplo', data_emissao: new Date().toISOString().split('T')[0], descricao: 'Item comprado', quantidade: 10, unidade: 'unidade', valor_unitario: 5, valor_total: 50 }],
      'Instruções': [
        { Campo: 'numero', Orientação: 'Número da nota ou documento de compra.' },
        { Campo: 'fornecedor_nome', Orientação: 'Nome do fornecedor para rastreio.' },
        { Campo: 'quantidade/valor_unitario', Orientação: 'Usados para alimentar estoque e custo médio.' },
      ],
    }, 'modelo-importacao-nota-estoque.xlsx', { title: 'Modelo de importação — notas e abastecimento' })
  },
}
