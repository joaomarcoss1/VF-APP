    ...
    <main className="relative min-h-dvh overflow-hidden bg-[#020617] text-white vf-theme-transition">
      ...
      <section className="relative mx-auto flex min-h-dvh max-w-7xl flex-col justify-between px-5 py-6 md:px-8 md:py-10">
        <header className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[var(--vf-card)] text-blue-700 shadow-xl shadow-blue-900/40"><Sparkles size={24} /></div><div><strong className="block text-lg">VF Nexus</strong><span className="text-[11px] font-black uppercase tracking-[.25em] text-blue-200">{ramo?.nome ?? 'Atendimento'}</span></div></div>
          <div className="flex flex-wrap items-center gap-2"><ThemeToggle /><a href="/atendimento/login-funcionario" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/15 bg-[var(--vf-card)]/10 px-4 py-3 text-sm font-black text-white backdrop-blur transition hover:bg-[var(--vf-card)]/15"><ShieldCheck size={17} /> Login do funcionário</a></div>
        </header>
        <div className="py-10 md:py-16">
          ...
          <p className="mt-5 max-w-2xl text-base font-semibold leading-8 text-[var(--vf-text3)] md:text-lg">Aparecem somente setores liberados para o ramo e para o funcionário. Atendimento, cozinha, bar/drinks e caixa ficam separados por função.</p>
          {staffName && <div className="mt-5 inline-flex rounded-2xl border border-white/10 bg-[var(--vf-card)]/10 px-4 py-3 text-sm font-bold text-slate-200">Operador: <strong className="ml-2 text-white">{staffName}</strong><span className="ml-2 rounded-full bg-blue-500/20 px-2 text-blue-100">{staffSector}</span></div>}
          <div className="mt-9 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {visibleSectors.map((sector, index) => {
              const Icon = sector.icon
              return <button key={sector.id} onClick={() => go(sector.id, sector.href)} className="group relative overflow-hidden rounded-[30px] border border-white/12 bg-[var(--vf-card)]/[.08] p-5 text-left shadow-2xl shadow-black/30 backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:bg-[var(--vf-card)]/[.13] active:scale-[.985]" style={{ animation: `vf-sector-in .42s ease ${index * 70}ms both` }}><div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${sector.tone}`} /><div className={`grid h-16 w-16 place-items-center rounded-3xl bg-gradient-to-br ${sector.tone} shadow-lg`}><Icon size={31} /></div><h2 className="mt-6 text-xl font-black leading-tight">{sector.title}</h2><p className="mt-3 min-h-[76px] text-sm font-semibold leading-6 text-[var(--vf-text3)]">{sector.subtitle}</p><span className="mt-4 inline-flex rounded-full bg-[var(--vf-card)] px-4 py-2 text-xs font-black text-[var(--vf-text)] transition group-hover:scale-105">Acessar setor</span></button>
            })}
          </div>
          {visibleSectors.length === 0 && <div className="mt-8 rounded-[28px] border border-white/10 bg-[var(--vf-card)]/10 p-5 text-sm font-semibold text-slate-200">Nenhum setor operacional liberado para este usuário. Solicite ao gerente/admin ou ao Admin Master.</div>}
        </div>
        <footer className="flex flex-col justify-between gap-3 border-t border-white/10 pt-5 text-xs font-semibold text-[var(--vf-text3)] md:flex-row"><span>VF Nexus · NexLabs</span><span>Setores ocultos quando não liberados</span></footer>
      </section>
    </main>
  )
}
