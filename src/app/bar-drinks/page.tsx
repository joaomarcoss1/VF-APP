  ...
  return (
    <OperationalShell sector="bar_drinks" title="Bar / Drinks" subtitle="Bebidas, drinks e itens do bar separados da cozinha" actions={<button onClick={load} className="vf-btn vf-btn-ghost"><RotateCcw size={15} /> Atualizar</button>}>
      ...
      <section className="mt-6 rounded-[30px] border border-slate-200 bg-white p-4 shadow-sm md:p-5">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center"><div><h2 className="text-xl font-black text-slate-950">Fila do Bar / Drinks</h2><p className="text-sm font-semibold text-slate-500">Vermelho: novo · Amarelo: em preparo · Verde: pronto para retirada.</p></div><div className="flex flex-wrap gap-2">{filters.map((item) => <button key={item.id} onClick={() => setFilter(item.id)} className={`rounded-full px-4 py-2 text-xs font-black transition ${filter === item.id ? 'bg-slate-950 text-white' : 'bg-slate-100 text-slate-600 hover:bg-emerald-50 hover:text-emerald-700'}`}>{item.label}</button>)}</div></div>
        ...
        {!loading && filtered.length === 0 && <div className="vf-empty mt-5 text-center"><strong>Nenhum pedido nessa fila.</strong><p className="text-sm text-slate-500">Aguardando bebidas e drinks do atendimento.</p></div>}
      </section>
    </OperationalShell>
  )
}
