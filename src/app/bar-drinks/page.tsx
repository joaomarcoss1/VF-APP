...
import { Clock3, Wine, Flame, RotateCcw } from 'lucide-react'
...
export default function BarDrinksPage() {
  ...
  const { notifications, markAsRead } = useRestaurantNotifications('bar_drinks')

  async function load() {
    setLoading(true)
    setOrders(await RestauranteService.listarPedidosBarDrinks('todos'))
    setLoading(false)
  }
  ...
  async function change(orderId: string, status: OrderStatus) {
    try {
      ...
      toast.success(status === 'em_preparo' ? 'Preparo iniciado.' : status === 'pronto' ? 'Drink/bebida marcado como pronto.' : 'Pedido atualizado.')
      await load()
    } catch (error: any) { toast.error(error.message ?? 'Erro ao atualizar pedido.') }
  }

  return (
    <OperationalShell sector="bar_drinks" title="Bar / Drinks" subtitle="Bebidas, drinks e itens do bar separados da cozinha" actions={<button onClick={load} className="vf-btn vf-btn-ghost"><RotateCcw size={15} /> Atualizar</button>}>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-[26px] border border-emerald-200 bg-emerald-50 p-5"><div className="flex items-center gap-3"><Wine className="text-emerald-600" /><span className="text-sm font-black uppercase text-emerald-700">Prontos</span></div><strong className="mt-3 block text-4xl font-black text-emerald-700">{counters.pronto}</strong></div>
      </section>
      <section className="mt-6 rounded-[30px] border border-[var(--vf-border)] bg-[var(--vf-card)] p-4 shadow-sm md:p-5">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center"><div><h2 className="text-xl font-black text-[var(--vf-text)]">Fila do Bar / Drinks</h2><p className="text-sm font-semibold text-[var(--vf-text3)]">Vermelho: novo · Amarelo: em preparo · Verde: pronto para retirada.</p></div><div className="flex flex-wrap gap-2">{filters.map((item) => <button key={item.id} onClick={() => setFilter(item.id)} className={`rounded-full px-4 py-2 text-xs font-black transition ${filter === item.id ? 'bg-slate-950 text-white' : 'bg-[var(--vf-surface2)] text-[var(--vf-text2)] hover:bg-emerald-50 hover:text-emerald-700'}`}>{item.label}</button>)}</div></div>
        ...
        {!loading && filtered.length === 0 && <div className="vf-empty mt-5 text-center"><strong>Nenhum pedido nessa fila.</strong><p className="text-sm text-[var(--vf-text3)]">Aguardando bebidas e drinks do atendimento.</p></div>}
      </section>
    </OperationalShell>
  )
}
