  ...
  return <div className="vf-fadein">
    ...
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <Card className="p-4"><div className="text-xs text-[var(--vf-text2)] uppercase">Ativos</div><div className="text-2xl text-[var(--vf-success)] font-semibold">{resumo.ativos}</div></Card>
        <Card className="p-4"><div className="text-xs text-[var(--vf-text2)] uppercase">Gestão/Financeiro</div><div className="text-2xl text-[var(--vf-primary)] font-semibold">{resumo.gestores}</div></Card>
      </div>
    ...
    <Modal open={modal} onClose={close} title={editing ? 'Editar membro' : 'Novo membro'} size="lg">
      <div className="space-y-4">
        ...
        <Field label="Permissões"><div className="grid grid-cols-2 sm:grid-cols-3 gap-2">{PERMISSOES.map(p => <button type="button" key={p.key} onClick={() => togglePerm(p.key)} className={`rounded-xl border px-3 py-2 text-xs text-left ${form.permissoes.includes(p.key) ? 'bg-[color-mix(in_srgb,var(--vf-primary)_12%,transparent)] border-[color-mix(in_srgb,var(--vf-primary)_34%,transparent)] text-[var(--vf-secondary)]' : 'border-[color-mix(in_srgb,var(--vf-muted)_18%,transparent)] text-[var(--vf-text2)]'}`}>{p.label}</button>)}</div></Field>
