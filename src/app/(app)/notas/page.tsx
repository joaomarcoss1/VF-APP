...
import { Alert, Badge, Button, ButtonLink, Card, Empty, Skeleton } from '@/components/ui'
    ...
    <div className="vf-fadein">
      ...
      <div className="vf-page-container space-y-5">
        ...
        <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2"><Button variant="secondary" onClick={() => NotasFiscaisService.exportarModeloCSV()}>Modelo Excel</Button><ButtonLink href="/configuracoes" variant="secondary" fullWidth>Configurar fiscal</ButtonLink><ButtonLink href="/estoque" fullWidth>Lançar nota</ButtonLink></div>
        </div>

        <Card className="overflow-hidden">
          {isLoading ? <div className="p-4"><Skeleton className="h-40" /></div> : (notas?.length ?? 0) === 0 ? <Empty icon="🧾" title="Nenhuma nota registrada" description="Lance uma nota no módulo Estoque para abastecer insumos ou produtos finais." action={<ButtonLink href="/estoque">Lançar nota</ButtonLink>} /> : <>
