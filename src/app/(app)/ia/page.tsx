      ...
      <div className="flex flex-1 min-h-0 gap-0">
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full py-12 text-center">
                <div className="w-16 h-16 rounded-full bg-[color-mix(in_srgb,var(--vf-secondary)_12%,transparent)] border border-[color-mix(in_srgb,var(--vf-secondary)_30%,transparent)] flex items-center justify-center text-2xl mb-4">✦</div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-xl">
                  {QUICK_PROMPTS.map(p => (
                    <button key={p} onClick={() => sendMessage(p)}
                      className="text-left p-3 rounded-lg bg-[var(--vf-surface2)] border border-[var(--vf-border)] hover:border-[color-mix(in_srgb,var(--vf-secondary)_30%,transparent)] text-[12px] text-[var(--vf-text2)] hover:text-[var(--vf-text)] transition-all">
                      {p}
                    </button>
                  ))}
                </div>
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[80%] px-4 py-3 rounded-2xl text-[13px] leading-relaxed whitespace-pre-wrap ${
                  msg.role === 'user'
                    ? 'bg-[color-mix(in_srgb,var(--vf-secondary)_12%,transparent)] border border-[color-mix(in_srgb,var(--vf-secondary)_25%,transparent)] text-[var(--vf-text)] rounded-br-sm'
                    : 'bg-[var(--vf-surface2)] border border-[color-mix(in_srgb,var(--vf-border)_45%,transparent)] text-[var(--vf-text)] rounded-bl-sm'
                }`}>
                  {msg.content}
                </div>
                {msg.role === 'user' && (
                  <div className="w-7 h-7 rounded-full bg-[var(--vf-surface2)] border border-[color-mix(in_srgb,var(--vf-secondary)_25%,transparent)] flex items-center justify-center text-[var(--vf-primary)] text-xs font-bold flex-shrink-0 mt-1">U</div>
                )}
              </div>
            ))}
              ...
              <div className="flex gap-3 justify-start">
                ...
                <div className="px-4 py-3 rounded-2xl rounded-bl-sm bg-[var(--vf-surface2)] border border-[color-mix(in_srgb,var(--vf-border)_45%,transparent)]">
                  <div className="flex items-center gap-1.5">
                    {[0,1,2].map(i => (
                      <span key={i} className="w-1.5 h-1.5 rounded-full bg-[var(--vf-secondary)] animate-bounce" style={{ animationDelay: `${i*150}ms` }} />
                    ))}
                  </div>
                </div>
              </div>
        ...
        <div className="hidden lg:flex flex-col w-64 border-l border-[var(--vf-border)] p-4 space-y-4 overflow-y-auto">
          ...
          <Card className="p-3 space-y-2">
            ...
            <div className="text-[13px] text-[var(--vf-text2)]">Lucro: <span className="text-[var(--vf-success)] font-medium">{fmtCurrency(dashboard?.lucro_mes ?? 0)}</span></div>
          ...
          <div className="space-y-1">
            {QUICK_PROMPTS.slice(0, 4).map(p => (
              <button key={p} onClick={() => sendMessage(p)}
                className="w-full text-left text-[11px] text-[var(--vf-text3)] hover:text-[var(--vf-primary)] transition-colors py-1 border-b border-[color-mix(in_srgb,var(--vf-border)_30%,transparent)] last:border-0">
                {p}
              </button>
            ))}
          </div>
