...
import { EventosService, ProdutosService, IdentidadeService } from '@/services'
...
export default function EventosPage() {
  ...
  const { data: identidade } = useQuery({
    queryKey: ['identidade-global'],
    queryFn: IdentidadeService.obter,
    retry: false,
    staleTime: 60_000,
  })
  ...
  async function exportarEvento(evento: Evento, tipo: 'pdf' | 'excel') {
    ...
    if (tipo === 'pdf') await exports.exportarEventoPDF(evento, identidade ?? undefined)
        ...
        {loadingEventos ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {eventos!.map(evento => {
              ...
              return (
                <Card key={evento.id} className="p-4 flex flex-col gap-4 hover:border-[color-mix(in_srgb,var(--vf-secondary)_42%,transparent)] transition-colors">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-lg bg-[var(--vf-surface2)] p-3">
                      ...
                      <div className="text-sm font-semibold text-[var(--vf-error)] mt-1">{fmtCurrency(evento.custo_total)}</div>
                    </div>
                    ...
                    <div className="rounded-lg bg-[color-mix(in_srgb,var(--vf-success)_6%,transparent)] p-3">
                      <div className="text-[10px] text-[var(--vf-text3)] uppercase">Lucro estimado</div>
                      <div className="text-sm font-semibold text-[var(--vf-success)] mt-1">{fmtCurrency(evento.lucro_estimado)}</div>
                    </div>
                    <div className="rounded-lg bg-[var(--vf-surface2)] p-3">
                ...
                return (
                  <Card key={item.produto_id} className="p-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-4">
                      ...
                      <div className="rounded-lg bg-[color-mix(in_srgb,var(--vf-error)_6%,transparent)] p-3">
                        <div className="text-[10px] text-[var(--vf-text3)] uppercase">Custo do item</div>
                        <div className="text-sm font-semibold text-[var(--vf-error)] mt-1">{fmtCurrency(calculado?.custo_total ?? 0)}</div>
                      </div>
                    </div>
                  </Card>
                )
                ...
                <tbody>
                  {resultado.cenarios.map(cenario => (
                    <tr key={cenario.margem}>
                      ...
                      <td className="text-[var(--vf-success)]">{fmtCurrency(cenario.lucro_estimado)}</td>
