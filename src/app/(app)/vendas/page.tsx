    ...
    <div className="vf-fadein">
      ...
      <div className="p-4 md:p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <Card className="p-4"><div className="text-xs text-[var(--vf-text2)] uppercase">Lucro estimado</div><div className="text-2xl text-[var(--vf-success)] font-semibold">{fmtCurrency(resumo.lucro)}</div></Card>
        ...
        <Card className="overflow-hidden">
          {isLoading ? <div className="p-4"><Skeleton className="h-32" /></div> : (vendas?.length ?? 0) === 0 ? (
            ...
            <div className="md:hidden p-3 space-y-3">{vendas!.map(v => <Card key={v.id} className="p-3 bg-[var(--vf-surface2)]">
              ...
              <div className="grid grid-cols-3 gap-2 mt-3 text-sm"><div><span className="text-[var(--vf-text3)] block text-xs">Itens</span><b>{v.itens?.length ?? 1}</b></div><div><span className="text-[var(--vf-text3)] block text-xs">Total</span><b className="text-[var(--vf-secondary)]">{fmtCurrency(v.total)}</b></div><div><span className="text-[var(--vf-text3)] block text-xs">Lucro</span><b className="text-[var(--vf-success)]">{fmtCurrency(v.lucro)}</b></div></div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Nova venda profissional" size="xl">
        <div className="space-y-5">
          ...
          <Card className="p-4 grid grid-cols-2 md:grid-cols-6 gap-3 bg-[var(--vf-surface2)]">
            ...
            <div><span className="text-xs text-[var(--vf-text3)]">Pagamentos</span><b className={Math.abs(totalPagamentos - total) <= 0.01 ? 'block text-[var(--vf-success)]' : 'block text-[var(--vf-error)]'}>{fmtCurrency(totalPagamentos)}</b></div>
