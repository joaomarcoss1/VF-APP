    ...
    <OperationalShell sector="caixa" title="Fechamento do caixa" subtitle="Conferência de valores, divergências e impressão">
      <section className="grid gap-5 lg:grid-cols-[1fr,440px]">
        <div className="rounded-[30px] border border-[var(--vf-border)] bg-[var(--vf-card)] p-5 shadow-sm">
          <h2 className="text-xl font-black text-[var(--vf-text)]">Resumo do período</h2>
          <p className="text-sm font-semibold text-[var(--vf-text3)]">Confira vendas e valores antes de encerrar a operação.</p>
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            <div className="rounded-2xl bg-[var(--vf-surface2)] p-4"><span className="text-xs font-black uppercase text-[var(--vf-text3)]">Valor de abertura</span><strong className="mt-2 block text-2xl font-black">{formatCurrency(cash?.valor_abertura ?? 0)}</strong></div>
        ...
        <div className="rounded-[30px] border border-[var(--vf-border)] bg-[var(--vf-card)] p-5 shadow-sm">
          <h2 className="text-xl font-black text-[var(--vf-text)]">Conferência</h2>
          <label className="mt-5 block rounded-2xl border border-[var(--vf-border)] p-4"><span className="text-xs font-black uppercase text-[var(--vf-text3)]">Valor informado pelo operador</span><input value={valor} onChange={(e) => setValor(Number(e.target.value))} type="number" step="0.01" className="mt-2 w-full border-0 bg-transparent text-3xl font-black outline-none" /></label>
          <label className="mt-4 block"><span className="text-xs font-black uppercase text-[var(--vf-text3)]">Observação</span><textarea value={obs} onChange={(e) => setObs(e.target.value)} className="vf-input mt-2 min-h-28" placeholder="Obrigatória se houver divergência." /></label>
