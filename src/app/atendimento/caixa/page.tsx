  ...
  if (!cash && !loading) {
    return (
      <OperationalShell sector="caixa" title="Caixa" subtitle="Abra o caixa antes de receber pagamentos">
        <section className="mx-auto max-w-xl rounded-[32px] border border-slate-200 bg-white p-6 shadow-xl">
          <div className="grid h-16 w-16 place-items-center rounded-3xl bg-emerald-50 text-emerald-700"><Banknote size={30} /></div>
          <h2 className="mt-5 text-2xl font-black text-slate-950">Abrir caixa</h2>
          <p className="mt-2 text-sm font-semibold leading-6 text-slate-500">Informe o valor inicial para iniciar os recebimentos do dia.</p>
          <label className="mt-6 block rounded-2xl border border-slate-200 p-4"><span className="text-xs font-black uppercase text-slate-500">Valor inicial</span><input value={openingValue} onChange={(e) => setOpeningValue(Number(e.target.value))} type="number" step="0.01" className="mt-2 w-full border-0 bg-transparent text-3xl font-black outline-none" /></label>
  ...
  return (
    <OperationalShell sector="caixa" title="Caixa" subtitle="Comandas, pagamentos, pedidos e fechamento" actions={<a href="/atendimento/caixa/fechamento" className="vf-btn vf-btn-secondary"><Calculator size={15} /> Fechar caixa</a>}>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <div className="vf-card p-5"><span className="text-xs font-black uppercase text-slate-500">Aguardando fechamento</span><strong className="mt-2 block text-3xl font-black text-amber-600">{summary.aguardando}</strong></div>
        <div className="vf-card p-5"><span className="text-xs font-black uppercase text-slate-500">Em atendimento</span><strong className="mt-2 block text-3xl font-black text-blue-600">{summary.abertas}</strong></div>
        <div className="vf-card p-5"><span className="text-xs font-black uppercase text-slate-500">Pedidos prontos</span><strong className="mt-2 block text-3xl font-black text-emerald-600">{summary.pedidosProntos}</strong></div>
        <div className="vf-card p-5"><span className="text-xs font-black uppercase text-slate-500">Total em aberto</span><strong className="mt-2 block text-3xl font-black text-slate-950">{formatCurrency(summary.totalAberto)}</strong></div>
      </section>
      ...
      <section className="mt-6 grid gap-5 xl:grid-cols-[370px,1fr,320px]">
        <div className="rounded-[30px] border border-slate-200 bg-white p-4 shadow-sm md:p-5">
          <h2 className="text-lg font-black text-slate-950">Comandas</h2>
          <p className="text-sm font-semibold text-slate-500">Somente o caixa finaliza pagamento e libera mesa.</p>
          <div className="mt-4 space-y-3">
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => setSelected(tab)} className={`w-full rounded-[22px] border p-4 text-left transition ${selected?.id === tab.id ? 'border-blue-300 bg-blue-50' : tab.status === 'aguardando_fechamento' ? 'border-amber-300 bg-amber-50 animate-[vf-attention_1.9s_ease-in-out_infinite]' : 'border-slate-200 bg-slate-50 hover:border-blue-200'}`}>
                <div className="flex items-start justify-between gap-2"><div><strong className="block text-slate-950">{tab.mesa?.nome ?? `Comanda #${tab.codigo}`}</strong><span className="text-xs font-bold text-slate-500">{tab.cliente_nome ?? 'Cliente não informado'} · {tab.itens?.length ?? 0} itens</span></div><StatusBadge status={tab.status} /></div>
          ...
          {!selected ? <div className="vf-empty">Selecione uma comanda para receber.</div> : (
            <>
              <div className="flex flex-col justify-between gap-3 md:flex-row md:items-start">
                <div><h2 className="text-xl font-black text-slate-950">Comanda #{selected.codigo}</h2><p className="text-sm font-semibold text-slate-500">{selected.mesa?.nome ?? 'Venda balcão'} · {selected.cliente_nome ?? 'Cliente não informado'}</p></div>
              ...
              <div className="mt-5 space-y-3">
                {(selected.itens ?? []).map((item) => <div key={item.id} className="flex justify-between gap-3 rounded-2xl bg-slate-50 p-3 text-sm"><div><strong>{item.quantidade}x {item.nome_produto}</strong>{item.observacao && <p className="text-xs font-semibold text-slate-500">Obs: {item.observacao}</p>}</div><strong>{formatCurrency(item.total)}</strong></div>)}
              </div>
              <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm">
                ...
                <div className="mt-3 flex justify-between border-t border-slate-200 pt-3 text-xl"><span className="font-black">Total</span><strong className="text-blue-600">{formatCurrency(selected.total)}</strong></div>
              </div>
        ...
        <aside className="space-y-4">
          <div className="rounded-[30px] border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-lg font-black text-slate-950">Resumo do caixa</h2>
          ...
          <div className="rounded-[30px] border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-lg font-black text-slate-950">Pedidos da cozinha</h2>
            <div className="mt-3 space-y-2">
              ...
              {orders.filter((o) => o.status === 'pronto').length === 0 && <p className="text-sm font-semibold text-slate-500">Nenhum pedido pronto.</p>}
            </div>
          </div>
        </aside>
      </section>
