  ...
  return (
    <OperationalShell sector="atendimento" title="VF Nexus Atendimento" subtitle="Mesas, comandas, pedidos e fechamento rápido" actions={<a href="/atendimento/funcionarios" className="vf-btn vf-btn-ghost hidden md:inline-flex"><Settings size={15} /> Configurar</a>}>
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-4">
        <div className="vf-card p-5"><span className="text-xs font-black uppercase text-[var(--vf-text3)]">Comandas abertas</span><strong className="mt-2 block text-3xl font-black text-[var(--vf-text)]">{counters.abertas}</strong></div>
        <div className="vf-card p-5"><span className="text-xs font-black uppercase text-[var(--vf-text3)]">Aguardando caixa</span><strong className="mt-2 block text-3xl font-black text-amber-600">{counters.aguardando}</strong></div>
        <div className="vf-card p-5"><span className="text-xs font-black uppercase text-[var(--vf-text3)]">Pedidos prontos</span><strong className="mt-2 block text-3xl font-black text-emerald-600">{counters.prontas}</strong></div>
        <div className="vf-card p-5"><span className="text-xs font-black uppercase text-[var(--vf-text3)]">Total em aberto</span><strong className="mt-2 block text-3xl font-black text-blue-600">{formatCurrency(counters.total)}</strong></div>
      </section>

      <section id="mesas" className="mt-6 rounded-[30px] border border-[var(--vf-border)] bg-[var(--vf-card)] p-4 shadow-sm md:p-5">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div><h2 className="text-xl font-black text-[var(--vf-text)]">Mesas do salão</h2><p className="text-sm font-semibold text-[var(--vf-text3)]">Toque em uma mesa livre para abrir ou ocupada para continuar a comanda.</p></div>
          <div className="flex flex-wrap gap-2">{filters.map((item) => <button key={item.id} onClick={() => setFilter(item.id)} className={`rounded-full px-4 py-2 text-xs font-black transition ${filter === item.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'bg-[var(--vf-surface2)] text-[var(--vf-text2)] hover:bg-blue-50 hover:text-blue-700'}`}>{item.label}</button>)}</div>
        </div>
      ...
      <section id="comandas" className="mt-6 rounded-[30px] border border-[var(--vf-border)] bg-[var(--vf-card)] p-4 shadow-sm md:p-5">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center"><div><h2 className="text-xl font-black text-[var(--vf-text)]">Comandas em andamento</h2><p className="text-sm font-semibold text-[var(--vf-text3)]">Visualização rápida de mesa, cliente, itens, status e valor.</p></div><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--vf-text3)]" size={16} /><input placeholder="Buscar comanda" className="vf-input pl-10" /></div></div>
        <div className="mt-5 grid gap-3 lg:grid-cols-2">
          {tabs.map((tab) => <button key={tab.id} onClick={() => router.push(`/atendimento/comanda/${tab.id}`)} className="flex items-center justify-between gap-4 rounded-[24px] border border-[var(--vf-border)] bg-[var(--vf-surface2)] p-4 text-left transition hover:border-blue-200 hover:bg-blue-50"><div className="min-w-0"><strong className="block text-[var(--vf-text)]">{tab.tipo === 'mesa' ? tab.mesa?.nome ?? `Mesa ${tab.mesa?.numero ?? ''}` : 'Venda balcão'} · #{tab.codigo}</strong><span className="mt-1 flex items-center gap-2 text-xs font-bold text-[var(--vf-text3)]">{tab.cliente_nome ?? 'Cliente não informado'} · {tab.itens?.length ?? 0} itens</span></div><div className="text-right"><StatusBadge status={tab.status} /><strong className="mt-2 block text-blue-600">{formatCurrency(tab.total)}</strong></div></button>)}
          {tabs.length === 0 && <div className="vf-empty lg:col-span-2"><ShoppingBag size={22} /><strong className="mt-2 block">Nenhuma comanda aberta.</strong><p className="text-sm text-[var(--vf-text3)]">Abra uma nova comanda para iniciar o atendimento.</p></div>}
        </div>
      </section>
      ...
      {modalOpen && <div className="fixed inset-0 z-50 grid place-items-end bg-slate-950/60 p-0 backdrop-blur-sm md:place-items-center md:p-4">
        <div className="w-full max-w-2xl rounded-t-[32px] bg-[var(--vf-card)] p-5 shadow-2xl md:rounded-[32px]">
          <div className="flex items-start justify-between gap-3"><div><h2 className="text-2xl font-black text-[var(--vf-text)]">Nova comanda</h2><p className="text-sm font-semibold text-[var(--vf-text3)]">Escolha mesa enumerada ou venda balcão.</p></div><button onClick={() => setModalOpen(false)} className="grid h-10 w-10 place-items-center rounded-2xl bg-[var(--vf-surface2)] text-[var(--vf-text2)]"><X size={18} /></button></div>
          <div className="mt-5 grid grid-cols-2 gap-3"><button onClick={() => setMode('mesa')} className={`rounded-2xl border p-4 text-left font-black ${mode === 'mesa' ? 'border-blue-300 bg-blue-50 text-blue-700' : 'border-[var(--vf-border)] bg-[var(--vf-surface2)]'}`}>Atendimento em mesa</button><button onClick={() => setMode('balcao')} className={`rounded-2xl border p-4 text-left font-black ${mode === 'balcao' ? 'border-blue-300 bg-blue-50 text-blue-700' : 'border-[var(--vf-border)] bg-[var(--vf-surface2)]'}`}>Venda balcão</button></div>
          {mode === 'mesa' && <label className="mt-4 block"><span className="text-xs font-black uppercase text-[var(--vf-text3)]">Mesa livre</span><select value={selectedMesaId} onChange={(e) => setSelectedMesaId(e.target.value)} className="vf-input mt-2"><option value="">Escolha uma mesa</option>{freeTables.map((m) => <option key={m.id} value={m.id}>{m.nome ?? `Mesa ${m.numero}`} · {m.capacidade ?? 4} lugares</option>)}</select></label>}
          <div className="mt-4 grid gap-3 md:grid-cols-[1fr,160px]"><label><span className="text-xs font-black uppercase text-[var(--vf-text3)]">Cliente opcional</span><input value={cliente} onChange={(e) => setCliente(e.target.value)} className="vf-input mt-2" placeholder="Nome do cliente" /></label><label><span className="text-xs font-black uppercase text-[var(--vf-text3)]">Pessoas</span><input value={pessoas} onChange={(e) => setPessoas(Number(e.target.value))} className="vf-input mt-2" type="number" min={1} /></label></div>
