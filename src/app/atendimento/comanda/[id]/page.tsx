...
export default function ComandaAtendimentoPage() {
  ...
  const itemsPanel = (
    <div className="rounded-[30px] border border-slate-200 bg-white p-4 shadow-sm md:p-5">
      <div className="flex items-center justify-between"><h2 className="text-lg font-black text-slate-950">Itens da comanda</h2><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-black text-blue-700">{itemCount} un.</span></div>
      <div className="mt-4 space-y-3">
        {(tab.itens ?? []).map((item) => <div key={item.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-3">
          <div className="flex items-start justify-between gap-3"><div><strong className="text-sm text-slate-950">{item.nome_produto}</strong>{item.observacao && <p className="text-xs font-semibold text-slate-500">Obs: {item.observacao}</p>}</div><strong className="text-sm text-slate-950">{formatCurrency(item.total)}</strong></div>
          <div className="mt-3 flex items-center justify-between gap-2"><StatusBadge status={item.status} /><div className="flex items-center gap-1 rounded-2xl bg-white p-1 ring-1 ring-slate-200"><button disabled={busyItem === item.id} onClick={() => changeQuantity(item, -1)} className="grid h-9 w-9 place-items-center rounded-xl bg-slate-100 text-slate-700"><Minus size={14} /></button><span className="min-w-10 text-center text-sm font-black">{item.quantidade}</span><button disabled={busyItem === item.id} onClick={() => changeQuantity(item, 1)} className="grid h-9 w-9 place-items-center rounded-xl bg-blue-600 text-white"><Plus size={14} /></button><button disabled={busyItem === item.id} onClick={() => editNote(item)} className="grid h-9 w-9 place-items-center rounded-xl bg-amber-50 text-amber-700"><StickyNote size={14} /></button><button disabled={busyItem === item.id} onClick={() => removeItem(item)} className="grid h-9 w-9 place-items-center rounded-xl bg-red-50 text-red-600"><Trash2 size={14} /></button></div></div>
        </div>)}
        {(tab.itens?.length ?? 0) === 0 && <div className="vf-empty text-sm font-semibold text-slate-500">Nenhum item lançado. Abra a aba Produtos e toque no item para adicionar.</div>}
      </div>
    </div>
  )
  ...
  const productsPanel = (
    <div className="rounded-[30px] border border-slate-200 bg-white p-4 shadow-sm md:p-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><h2 className="text-xl font-black text-slate-950">Adicionar produtos</h2><p className="text-sm font-semibold text-slate-500">Busca rápida por comidas, bebidas e porções.</p></div><div className="grid gap-2 md:grid-cols-[260px,220px]"><label className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar produto" className="vf-input pl-10" /></label><input value={observacao} onChange={(e) => setObservacao(e.target.value)} placeholder="Observação do próximo item" className="vf-input" /></div></div>
      <div className="mt-5 space-y-5">{Object.entries(grouped).map(([category, items]) => <div key={category}><h3 className="mb-3 text-sm font-black uppercase tracking-wide text-slate-500">{category}</h3><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{items.map((product) => <button key={product.id} onClick={() => add(product)} className="rounded-[22px] border border-slate-200 bg-slate-50 p-4 text-left transition hover:-translate-y-0.5 hover:border-blue-200 hover:bg-blue-50 active:scale-[.985]"><strong className="line-clamp-2 block min-h-[40px] text-sm font-black text-slate-950">{product.nome}</strong><div className="mt-3 flex items-center justify-between"><span className="rounded-full bg-white px-2 py-1 text-[11px] font-black uppercase text-slate-500">{product.setor_producao ?? 'balcão'}</span><span className="text-lg font-black text-blue-600">{formatCurrency(product.preco_venda ?? 0)}</span></div></button>)}</div></div>)}</div>
    </div>
  )

  const summaryPanel = (
    <div className="rounded-[30px] border border-slate-200 bg-white p-4 shadow-sm md:p-5">
      <div className="space-y-2 text-sm"><div className="flex justify-between"><span>Subtotal</span><strong>{formatCurrency(tab.subtotal)}</strong></div><div className="flex justify-between"><span>Taxa de serviço</span><strong>{formatCurrency(tab.taxa_servico)}</strong></div><div className="flex justify-between"><span>Desconto</span><strong>{formatCurrency(tab.desconto)}</strong></div><div className="border-t border-slate-200 pt-3 text-lg"><div className="flex justify-between"><span className="font-black">Total</span><strong className="text-blue-600">{formatCurrency(tab.total)}</strong></div></div></div>
  ...
  return (
    <OperationalShell sector="atendimento" title={`Comanda #${tab.codigo}`} subtitle={`${mesaLabel} · ${tab.cliente_nome ?? 'Cliente não informado'}`} actions={<StatusBadge status={tab.status} />}>
      <div className="mb-4 grid gap-3 md:grid-cols-4"><div className="vf-card p-4"><span className="text-xs font-black uppercase text-slate-500">Cliente</span><strong className="mt-1 block text-slate-950">{tab.cliente_nome ?? 'Não informado'}</strong></div><div className="vf-card p-4"><span className="text-xs font-black uppercase text-slate-500">Pessoas</span><strong className="mt-1 block text-slate-950">{tab.pessoas ?? '-'}</strong></div><div className="vf-card p-4"><span className="text-xs font-black uppercase text-slate-500">Mesa</span><strong className="mt-1 block text-slate-950">{mesaLabel}</strong></div><div className="vf-card p-4"><span className="text-xs font-black uppercase text-slate-500">Total</span><strong className="mt-1 block text-blue-600">{formatCurrency(tab.total)}</strong></div></div>
      <div className="vf-operational-mobile-tabs mb-4 md:hidden">{(['itens', 'produtos', 'resumo'] as MobileTab[]).map((item) => <button key={item} onClick={() => setMobileTab(item)} className={`rounded-2xl px-3 py-3 text-xs font-black capitalize ${mobileTab === item ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600'}`}>{item}</button>)}</div>
      ...
      <div className="vf-restaurant-totalbar bg-slate-950 p-4 text-white md:hidden"><div className="flex items-center justify-between"><span className="text-sm font-bold text-slate-300">Total</span><strong className="text-xl font-black">{formatCurrency(tab.total)}</strong></div></div>
    </OperationalShell>
  )
}
