...
## V14.4 — VF Nexus Entregas

- Adicionado módulo Entregas multiempresa.
- Portal do Entregador mobile-first.
- Finalização offline com IndexedDB e sincronização.
- Faturamento e recibos de entregadores.
- Google Maps externo para rotas.
- Migration 028 com RLS e tabelas de entregas.
...
## V12 — validação final comercial

- Reforço mobile/PWA para telas instaladas no celular.
- Conversão de exportações Excel para `.xlsx` real sem dependência externa.
- RPC transacional para venda completa: venda + itens + pagamentos + financeiro + estoque + auditoria.
- RPC transacional para nota/abastecimento.
- Estrutura fiscal ampliada para provedor/certificado/documentos fiscais/eventos.
- Reforço de tema global e CSS variables.
- Checklist e relatório final V12 adicionados.

## V13 — Mobile App Real e Relatórios/Catálogos Premium

- Redesenho da experiência mobile/PWA para funcionamento como aplicativo instalado.
- Header mobile com logo, título e menu touch-friendly.
- Bottom navigation com safe-area e área de toque adequada.
- Regras globais para evitar zoom, overflow horizontal e telas desktop comprimidas.
- Relatórios, cardápios, catálogos, comprovantes, fichas técnicas e eventos agora usam paleta e logo do cliente.
- Removidos fundos pretos fixos dos PDFs principais.
- Página de Cardápio recebeu prévia comercial do catálogo.
- Splash/login passam a priorizar símbolo NexLabs para melhor leitura em tamanho reduzido.


## V14 Comercial
- Adicionado PDV rápido mobile-first.
- Adicionado catálogo público com slug, QR Code e WhatsApp.
- Adicionada estrutura de planos SaaS/trial.
- Adicionada importação Excel com modelos.
- Adicionada central de suporte.
- Adicionada migration 024.
- Reforçadas melhorias mobile/PWA e relatórios com paleta/logo.

## V14.1 — Correções, offline, etiquetas e scanner

- Corrigido encoding corrompido em arquivos prioritários.
- Adicionado service worker com cache e fallback offline.
- Adicionada página `/offline`.
- Adicionada fila offline de vendas via IndexedDB.
- Venda transacional agora usa RPC `vf_registrar_venda_completa_v14_1` e bloqueia fallback parcial em produção.
- Criada migration `025_vf_nexus_v14_1_rls_offline_etiquetas.sql`.
- Adicionado módulo `/etiquetas` com geração de códigos, lotes, impressão e ZPL.
- Adicionado módulo `/scanner` com câmera, leitura por BarcodeDetector e entrada manual.
- Adicionada rota `/estoque/produto/[id]`.
- Importação XLSX/CSV passou a ler arquivo, validar e salvar no Supabase.
- Assinatura ganhou checkout inicial por provider configurável.

## V14.3 — Multiempresa seguro para testes iniciais

- Corrigido isolamento por empresa com migration 027.
- Criada tela `/login` profissional e redirecionamento por papel.
- Criada área `/master` para Admin Master NexLabs cadastrar empresas.
- Criada tela `/administracao` separando Admin da Empresa, Gerentes e Funcionários.
- Criado helper tenant em `src/services/_tenant.ts` e `MultiempresaService`.
- Ajustados services críticos para filtrar por `empresa_id`.
- Ajustado cache offline para separar produtos/vendas por empresa.
- Adicionados testes e documentação de segurança multiempresa.


## V15 Produção 10/10
- Migration 029 de segurança multiempresa/RLS.
- Funções SQL seguras para empresa, papéis, entregas, etiquetas e produtos.
- Guardas de rota por papel.
- Testes V15 de isolamento, RLS, RBAC, PDV, entregas, etiquetas e relatórios.
- Documentação final de deploy, Supabase e testes de piloto.


## V15.1 — Billing Stripe, Caixa e Operação Comercial

- Adicionado Stripe Checkout/Billing por assinatura.
- Adicionado webhook `/api/stripe/webhook`.
- Adicionado teste manual sem prazo para empresas.
- Adicionada isenção permanente de cobrança por empresa, exclusiva do Admin Master Global.
- Adicionada tela `/master/assinaturas`.
- Adicionada tela `/master/planos`.
- Adicionado diagnóstico técnico V15.1.
- Adicionada matriz visual de permissões.
- Adicionado fechamento de caixa operacional.
- Adicionado onboarding empresarial ampliado.
- Reforçado bloqueio sem empresa_id e bloqueio por billing.
