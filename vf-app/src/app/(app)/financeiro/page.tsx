...
function Mini({ label, value, strong }: { label: string; value: number; strong?: boolean }) {
  return <div className="p-3 rounded-2xl bg-[var(--vf-surface2)]"><div className="text-xs text-[var(--vf-text3)] uppercase">{label}</div><div className={`text-base ${strong ? 'font-bold' : 'font-semibold'} ${value >= 0 ? 'text-[#16A34A]' : 'text-[#DC2626]'}`}>{fmtCurrency(value)}</div></div>
}

function MiniPct({ label, value }: { label: string; value: number }) {
  return <div className="p-3 rounded-2xl bg-[var(--vf-surface2)]"><div className="text-xs text-[var(--vf-text3)] uppercase">{label}</div><div className={`text-base font-bold ${value >= 0 ? 'text-[#16A34A]' : 'text-[#DC2626]'}`}>{Number(value || 0).toFixed(1)}%</div></div>
}
