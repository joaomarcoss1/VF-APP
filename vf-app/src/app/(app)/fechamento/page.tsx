...
export default function FechamentoPage() {
  ...
  return <div className="vf-fadein">
    ...
    <div className="p-4 md:p-6 space-y-5">
      ...
      {isLoading ? <Skeleton className="h-52" /> : !fechamentos.length ? <Empty icon="🔐" title="Nenhum fechamento salvo" description="Faça seu primeiro fechamento diário para acompanhar caixa e pagamentos." /> : <Card className="overflow-hidden"><div className="hidden md:block overflow-x-auto"><table className="vf-table min-w-[880px]"><thead><tr><th>Data</th><th>Vendas</th><th>Receitas</th><th>Despesas</th><th>Saldo</th><th>Pix</th><th>Cartões</th><th>Status</th></tr></thead><tbody>{fechamentos.map((f: FechamentoDiario) => <tr key={f.id}><td>{new Date(`${f.data_fechamento}T00:00:00`).toLocaleDateString('pt-BR')}</td><td>{fmtCurrency(f.total_vendas)}</td><td className="text-[#3DAA6B]">{fmtCurrency(f.total_receitas)}</td><td className="text-[#D45050]">{fmtCurrency(f.total_despesas)}</td><td className={f.saldo_final >= 0 ? 'text-[#3DAA6B]' : 'text-[#D45050]'}>{fmtCurrency(f.saldo_final)}</td><td>{fmtCurrency(f.pix)}</td><td>{fmtCurrency(Number(f.cartao_credito || 0) + Number(f.cartao_debito || 0))}</td><td><Badge color={f.status === 'fechado' ? 'green' : 'amber'}>{f.status}</Badge></td></tr>)}</tbody></table></div><div className="md:hidden p-3 space-y-3">{fechamentos.map(f => <Card key={f.id} className="p-3 bg-[var(--vf-surface2)]"><div className="flex justify-between"><div><b className="text-[var(--vf-text)]">{new Date(`${f.data_fechamento}T00:00:00`).toLocaleDateString('pt-BR')}</b><div className="text-xs text-[var(--vf-text2)]">Vendas {fmtCurrency(f.total_vendas)}</div></div><Badge color={f.status === 'fechado' ? 'green' : 'amber'}>{f.status}</Badge></div><div className="grid grid-cols-2 gap-2 mt-3 text-sm"><span>Receitas: <b className="text-[#3DAA6B]">{fmtCurrency(f.total_receitas)}</b></span><span>Despesas: <b className="text-[#D45050]">{fmtCurrency(f.total_despesas)}</b></span><span>Pix: <b>{fmtCurrency(f.pix)}</b></span><span>Saldo: <b className={f.saldo_final >= 0 ? 'text-[#3DAA6B]' : 'text-[#D45050]'}>{fmtCurrency(f.saldo_final)}</b></span></div></Card>)}</div></Card>}
    </div>
  </div>
}
