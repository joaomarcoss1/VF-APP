...
export default function SimuladorPage() {
  ...
  const DiffBadge = ({ original, novo, money }: { original: number; novo: number; money?: boolean }) => {
    ...
    return (
      <span className={`text-[11px] font-medium ${diff > 0 ? 'text-[#3DAA6B]' : 'text-[#D45050]'}`}>
                  ...
                  <div className="flex gap-1 mt-1">
                    {[200,300,400,500].map(m => (
                      <button key={m} onClick={() => setMargemCustom(String(m))}
                        className={`text-[10px] px-2 py-0.5 rounded ${margemCustom===String(m)?'bg-[rgba(201,168,76,0.2)] text-[var(--vf-primary)]':'text-[var(--vf-text3)] hover:text-[var(--vf-text2)]'}`}>
                        {m}%
                      </button>
                    ))}
                  </div>
