...
export default function RelatoriosPage() {
  ...
  const exportPDF = async (tipo: 'fichas' | 'financeiro') => {
    ...
    if (tipo === 'financeiro') {
      ...
      for (const p of produtos) await exportarFichaTecnicaPDF(p)
          ...
          <Card className="p-4">
            ...
            {ranking.length ? ranking.map((p, i) => (
              <div key={p.id} className="flex items-center justify-between py-2 border-b border-[var(--vf-border)] last:border-0">
                ...
                <div className="text-[#3DAA6B] text-[13px] font-semibold">{fmtCurrency(p.lucro_bruto ?? 0)}</div>
              </div>
                  ...
                  <tbody>
                    {vendas.map(v => (
                      <tr key={v.id}>
                        ...
                        <td className="text-[#3DAA6B] font-medium">{fmtCurrency(v.lucro)}</td>
