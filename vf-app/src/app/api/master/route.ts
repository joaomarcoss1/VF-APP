...
async function companyModulesOverview(req: NextRequest) {
  ...
  const { data: empresaRows, error: empresaError } = await supabase
    ...
    .select('empresa_id,modulo,ativo,ordem,updated_at')
    .eq('empresa_id', empresaId)
  if (empresaError) throw empresaError

  const configs = (setorRows ?? []).map((row: any) => {
    const override = (empresaRows ?? []).find((r: any) => r.modulo === row.modulo)
    return {
...
async function updateCompanyModules(req: NextRequest) {
  ...
  const { error } = await supabase.from('empresa_modulos').upsert(payload, { onConflict: 'empresa_id,modulo' })
  if (error) throw error
  return NextResponse.json({ ok: true })
}
