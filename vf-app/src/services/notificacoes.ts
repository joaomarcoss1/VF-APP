...
export const NotificacoesService = {
  ...
  async listarCentral(limit = 80) {
    const { data, error } = await db().from('notificacoes_central').select('*').order('created_at', { ascending: false }).limit(limit)
  ...
  async criarCentral(form: { tipo: string; titulo: string; mensagem: string; prioridade?: 'baixa'|'media'|'alta'|'critica'; entidade?: string; entidade_id?: string }) {
    ...
    const payload = await withEmpresa({ ...form, prioridade: form.prioridade || 'media' } as AnyRecord)
  ...
  async marcarLida(id: string) {
    const { error } = await db().from('notificacoes_central').update({ lida: true, lida_em: new Date().toISOString() }).eq('id', id)
  ...
  async ativarNesteDispositivo(): Promise<void> {
    ...
    const userId = await getCurrentUserId(); const empresaId = await getEmpresaId()
    if (!userId) throw new Error('Sessão inválida. Entre novamente para ativar notificações.')
  ...
  async salvarPreferencias(config: Pick<Configuracoes, 'notificacao_agendamento_ativa' | 'notificacao_agendamento_antecedencia'>): Promise<void> { await ConfigService.salvar(config as Partial<Configuracoes>) },
}
