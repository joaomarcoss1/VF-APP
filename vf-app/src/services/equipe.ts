...
import { db, normalizeError, withEmpresa, assertPermission, getCurrentUserId, type AnyRecord } from './_base'
...
export const EquipeService = {
  ...
  async listarConvites(): Promise<any[]> {
    ...
    const { data, error } = await db().from('equipe_convites').select('*').order('created_at', { ascending: false })
  ...
  async cancelarConvite(id: string): Promise<void> {
    ...
    const { error } = await db().from('equipe_convites').update({ status: 'cancelado', updated_at: new Date().toISOString() }).eq('id', id)
  ...
  async listar(): Promise<EquipeUsuario[]> {
    const { data, error } = await db().from('equipe_usuarios').select('*').order('nome')
  ...
  async atualizar(id: string, form: Partial<EquipeUsuarioForm>): Promise<EquipeUsuario> {
    await assertPermission('equipe', 'editar')
    const { data, error } = await db().from('equipe_usuarios').update({ ...form, updated_at: new Date().toISOString() }).eq('id', id).select().single()
  ...
  async desativar(id: string): Promise<void> {
    ...
    const { error } = await db().from('equipe_usuarios').update({ status: 'inativo', updated_at: new Date().toISOString() }).eq('id', id)
