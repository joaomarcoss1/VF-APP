...
export const AgendamentosService = {
  async listar(inicio?: string, fim?: string): Promise<Agendamento[]> {
    let q = db().from('agendamentos').select('*, produto:produtos(id,nome,categoria,preco_venda,custo_total)').order('data_agendamento', { ascending: true }).order('hora_inicio', { ascending: true })
  ...
  async atualizar(id: string, form: Partial<AgendamentoForm>): Promise<Agendamento> {
    ...
    if (payload.valor !== undefined || payload.desconto !== undefined || payload.taxa_servico !== undefined) {
      const atual = await db().from('agendamentos').select('valor,desconto,taxa_servico').eq('id', id).maybeSingle()
  ...
  async proximos(limit = 20): Promise<Agendamento[]> {
    ...
    const { data, error } = await db().from('agendamentos').select('*').gte('data_agendamento', hoje).neq('status', 'cancelado').order('data_agendamento', { ascending: true }).order('hora_inicio', { ascending: true }).limit(limit)
