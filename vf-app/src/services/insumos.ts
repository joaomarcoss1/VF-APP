...
export const InsumosService = {
  async listar(search?: string): Promise<Insumo[]> {
    let q = db().from('insumos').select('*, categoria:categorias_insumos(id,nome,icone,cor), fornecedor:fornecedores(id,nome)').eq('ativo', true).order('nome')
  ...
  async buscarPorId(id: string): Promise<Insumo | null> {
    const { data, error } = await db().from('insumos').select('*, categoria:categorias_insumos(*), fornecedor:fornecedores(*)').eq('id', id).maybeSingle()
  ...
  async atualizar(id: string, form: Partial<InsumoForm>): Promise<Insumo> {
    ...
    const { data, error } = await db().from('insumos').update({ ...normalizeEmptyValues(form), updated_at: new Date().toISOString() }).eq('id', id).select().single()
  ...
  async excluir(id: string): Promise<void> {
    ...
    const { error } = await db().from('insumos').update({ ativo: false, updated_at: new Date().toISOString() }).eq('id', id)
  ...
  async alertasEstoque(): Promise<AlertaEstoque[]> {
    const { data, error } = await db().from('insumos').select('*').eq('ativo', true)
