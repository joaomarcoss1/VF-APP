...
export const CardapioService = {
  ...
  async atualizar(id: string, form: Partial<Cardapio>): Promise<Cardapio> {
    ...
    const { data, error } = await db().from('cardapios').update({ ...normalizeEmptyValues(form as AnyRecord), updated_at: new Date().toISOString() }).eq('id', id).select('*, itens:cardapio_itens(*)').single()
