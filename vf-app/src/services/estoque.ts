...
export const ProdutosEstoqueService = {
  async listar(): Promise<ProdutoEstoque[]> {
    const { data, error } = await db().from('produto_estoque').select('*, produto:produtos(*)').order('updated_at', { ascending: false })
...
export const EstoqueService = {
  ...
  async listar() {
    const { data, error } = await db().from('movimentacoes_estoque').select('*, insumo:insumos(nome,unidade_compra)').order('created_at', { ascending: false }).limit(120)
  ...
  async inventario() {
    const [produtos, insumos] = await Promise.all([ProdutosEstoqueService.listar(), db().from('insumos').select('*').eq('ativo', true)])
  ...
  async alertasRuptura() {
    const [produtos, insumos] = await Promise.all([ProdutosEstoqueService.alertasRuptura(), db().from('insumos').select('*').eq('ativo', true)])
...
export const InventariosEstoqueService = {
  async listar() {
    ...
    const { data, error } = await db().from('inventarios_estoque').select('*, itens:inventario_itens(*)').order('iniciado_em', { ascending: false })
  ...
  async salvarContagem(inventarioId: string, itens: Array<{ item_tipo: 'produto' | 'insumo' | 'variacao'; produto_id?: string; insumo_id?: string; variacao_id?: string; nome: string; saldo_sistema: number; saldo_contado: number; custo_medio?: number; justificativa?: string }>) {
    ...
    await db().from('inventario_itens').delete().eq('inventario_id', inventarioId)
    ...
    await db().from('inventarios_estoque').update({ status: 'em_contagem', updated_at: new Date().toISOString() }).eq('id', inventarioId)
