...
export const OrdensServicoService = {
  async listar(limit = 100): Promise<OrdemServico[]> {
    const { data, error } = await db().from('ordens_servico').select('*, cliente:clientes(*)').order('created_at', { ascending: false }).limit(limit)
  ...
  async buscarCompleta(id: string) {
    ...
    const [{ data: os, error }, checklist, materiais, fotos, assinaturas] = await Promise.all([
      db().from('ordens_servico').select('*, cliente:clientes(*)').eq('id', id).maybeSingle(),
      db().from('ordens_servico_checklist').select('*').eq('ordem_servico_id', id).order('ordem'),
      db().from('ordens_servico_materiais').select('*').eq('ordem_servico_id', id).order('created_at'),
      db().from('ordens_servico_fotos').select('*').eq('ordem_servico_id', id).order('created_at', { ascending: false }),
      db().from('ordens_servico_assinaturas').select('*').eq('ordem_servico_id', id).order('assinado_em', { ascending: false }),
    ])
  ...
  async atualizar(id: string, form: Partial<OrdemServico>): Promise<OrdemServico> {
    ...
    const { data, error } = await db().from('ordens_servico').update({ ...form, updated_at: new Date().toISOString() }).eq('id', id).select().single()
  ...
  async atualizarStatus(id: string, status: OrdemServico['status'], motivo?: string): Promise<void> {
    ...
    const { error } = await db().from('ordens_servico').update(patch).eq('id', id)
  ...
  async salvarChecklist(ordemServicoId: string, itens: Array<{ titulo: string; concluido?: boolean; ordem?: number }>) {
    ...
    await db().from('ordens_servico_checklist').delete().eq('ordem_servico_id', ordemServicoId)
